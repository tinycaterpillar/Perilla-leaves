...
class CRNN(nn.Module):
    def __init__(self, cnn_channel_num):
        ...
        # 기존 첫 번째 컨볼루션 레이어의 가중치 가져오기
        original_layer = self.cnn.conv1.weight.data

        # 새로운 컨볼루션 레이어 생성 (채널 수만 변경)
        self.cnn.conv1 = nn.Conv2d(cnn_channel_num, 64, kernel_size=7, stride=2, padding=3, bias=False)

        # 새 레이어의 가중치 초기화, 기존 가중치를 두 번 반복하여 사용
        with torch.no_grad():
            self.cnn.conv1.weight[:, :3] = original_layer
            self.cnn.conv1.weight[:, 3:] = original_layer
    ...
    def forward(self, x_cnn, x_len_cnn):
        ...
        output = output[:, -1, :]  # 최종 타임스텝의 CRNN 출력
...
class RNN(nn.Module):
    ...
    def forward(self, packed_rnn_input):
        packed_output, (h_n, c_n) = self.rnn(packed_rnn_input)
        output, _ = pad_packed_sequence(packed_output, batch_first=True)
        output = output[:, -1, :] # 최종 타임스텝의 RNN 출력
...
class AudioClassifier(nn.Module):
    ...
    def forward(self, x_cnn, x_len_cnn, packed_rnn_input):
        # CRNN을 통한 이미지 처리
        crnn_features = self.crnn(x_cnn, x_len_cnn)  # (batch_size, 512)

        # LSTM을 통한 시퀀셜 데이터 처리
        rnn_features = self.rnn(packed_rnn_input)  # (batch_size, 512)
