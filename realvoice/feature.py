...
def get_cnn_feature(df, train_mode=True):
    ...
    for _, row in tqdm(df.iterrows()):
        ...
        for sample in samples:
            ...
            spectral_contrast = librosa.feature.spectral_contrast(y=sample, sr=sr, n_fft=CONFIG.N_FFT, hop_length=CONFIG.HOP_LENGTH)
            diff = np.diff(spectral_contrast, axis=1)
            ...
            ax.set_axis_off()  # 축을 제거
            fig.add_axes(ax)
            librosa.display.specshow(diff, sr=sr, hop_length=CONFIG.HOP_LENGTH, x_axis=None, y_axis=None, fmax=8000, ax=ax)

            fig.canvas.draw()
            cont_img_array = np.frombuffer(fig.canvas.tostring_rgb(), dtype=np.uint8)
            cont_img_array = cont_img_array.reshape(fig.canvas.get_width_height()[::-1] + (3,))
            plt.close(fig)  # 그림을 닫아 리소스를 해제
            
            # 두 이미지를 채널 축을 따라 결합하여 (300, 300, 6) 형태로 만듦
            combined_image = np.concatenate([mel_img_array, cont_img_array], axis=2)
            cur_feature.append(combined_image)
